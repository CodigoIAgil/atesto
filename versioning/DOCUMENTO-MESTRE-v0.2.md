...
| Versão | 0.2 — rodada final de crítica antes do commit inicial |
| Status | EM REVISÃO — não publicado |
...
1. **Aciona:** somente incidente de severidade crítica ou alta em produção. Quem classifica é o Validador; quem autoriza o modo é o Operador.
2. **Portão compactado:** suíte crítica de regressão + scanner de segredos + auditoria de dependências. Assinatura e artefato de evidência **obrigatórios como sempre** — auditoria de emergência é quando mais se precisa de trilha.
3. **Dívida registrada:** o que foi pulado vira Pendência no STATUS.md, com prazo.
4. **Obrigação de retorno:** re-auditoria completa (F5 integral) no prazo definido no perfil. Fast-Track sem retorno agendado é violação do framework.

Rápido não significa cego; significa portão menor com dívida documentada.
...
**Anel 2 — O orquestrador confere, não confia.** A transição de fase é bloqueada se o artefato não existir, se o exit code divergir, se qualquer hash não bater ou se a execução não tiver ocorrido em ambiente efêmero elegível. Aprovação sem artefato = aprovação inexistente, por construção.
...
Dois blocos em paralelo tocam arquivos diferentes; o orquestrador lê `status/`; humanos e o Raciocínio leem o STATUS.md, que mantém as seções narrativas: o que estava sendo feito, estado atual validado, decisões (quem decidiu, quem vetou, por quê), vereditos da sessão, arquivos alterados, commit, pendências, riscos, próximo passo recomendado.
...
| Ambiente efêmero | Container/runner descartável, provisionado pelo orquestrador, fora da administração do Executor — único território onde nasce veredito oficial. |
